...
prediction = pd.read_csv(data_file, 
			 sep=',',
			 names = ["No.", "Chain", "residue_number", "residue_name", "Score"],
             skiprows=1)
## No.  Chain   Residue number  Residue name    Score
...
scored = prediction.loc[prediction['Score'] >= 0.5]
...
for i in range(len(scored)):
    try:
        if scored.loc[i,'residue_number']-scored.loc[i-1,'residue_number'] == 1:
            resid.append(scored.loc[i,'residue_number'])
    except:
        resid.append(scored.loc[i,'residue_number'])
        continue
...
for group in resid_grouped_filtered:
    sequence = []
    for n in group:
        sequence.append(scored.aa[scored.residue_number==n].values[0])
...
for group in resid_grouped_filtered:
    score = []
    for n in group:
        score.append(scored.Score[scored.residue_number==n].values[0])
