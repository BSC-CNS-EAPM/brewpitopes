...
prediction = pd.read_csv(data_file, 
			 sep='\t',
			 names = ["chainID", "resSeq", "resName", "score", "location"],
             skiprows=2)
...
scored = prediction.loc[prediction['score'] <= 0.064]
...
for i in range(len(scored)):
    try:
        if scored.loc[i,'resSeq']-scored.loc[i-1,'resSeq'] == 1:
            resid.append(scored.loc[i,'resSeqr'])
    except:
        resid.append(scored.loc[i,'resSeq'])
        continue
...
for group in resid_grouped_filtered:
    sequence = []
    for n in group:
        sequence.append(scored.aa[scored.resSeq==n].values[0])
...
for group in resid_grouped_filtered:
    score = []
    for n in group:
        score.append(scored.score[scored.resSeq==n].values[0])
